...
Open this vignette from within R with the command `vignette('Introduction to RFinfer')`.

## Overview

This vignette is an introductory overview of the `RFinfer` package for generating prediction variances from random forest predictions. The first section covers some of the theory behind the infinitesimal jackknife, how it is applied to a random forest to estimate the prediction variance, and its accuracy as studied using simulation experiments. The next section covers how to install the package in `R`. Next, we cover a simple example showing how to implement the main function, `rfPredVar()` to generate prediction with accompanying variance estimates.  Finally, we cover a more in depth example using NHANES data, comparing the different types of random forests and using plots to carry out some statistical inference.  These inferences are compared to a standard linear model.  Lastly, we summarize the functionality of the package, list future development directions, and discuss statistical learning situations that might benefit the most from this package.
...
This package is an implementation of the infinitesimal jackknife (IJ).

### Accuracy of IJ estimator

Cite my paper and the Wager arXiv paper too.
...
The package is currently not on CRAN, but can be installed from the R prompt:
...
Here, we can see that the random forest is generally less confident about its more inaccuracte predictions.  This can also be visualized by plotting the prediction variance as a function of the prediction error.
...
## Using Prediction Variance for Inference
...
We will use the `nhanesA` package to retreive information on BMI and some possibly related demographic characteristics.
...
- Currently, the function only works with regression forests, but in the future we plan to extend the implementation to classification and survival forests.
- Support for parallel computation and progress bars via CB::cb_apply
